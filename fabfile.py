...
def staging():
    ...
    # elastic IP 52.1.188.171
    env.master = 'ec2-52-1-188-171.compute-1.amazonaws.com'
    env.hosts = [env.master]
    env.key_filename = os.environ.get('HOME') + "/.ssh/enketo_staging.pem"
    initialize_env()
...
def production():
    ...
    env.master = 'ec2-52-28-150-143.eu-central-1.compute.amazonaws.com'
    env.hosts = [env.master]
    # env.user = 'ubuntu'
    # env.key_filename = os.environ.get('HOME') + "/.ssh/enketo-prd-keys.pem"
    initialize_env()
...
def install_salt(version, master=False, minion=False, restart=True):
    ...
    if install_master or install_minion:
        ...
        # then inspect it
        if not os.path.exists("install_salt.sh"):
            local("wget -O install_salt.sh https://bootstrap.saltstack.com")
